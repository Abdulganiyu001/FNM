...
$search = $_GET['search'] ?? '';
if ($search){
  $statement = $pdo->prepare('SELECT * FROM fem_proj2 WHERE surname LIKE :surname ORDER BY id DESC');
  $statement->bindValue(':surname', "%$search%");

}else{
  $statement = $pdo->prepare('SELECT * FROM fem_proj2 ORDER BY id DESC');
}
...
<html lang="en">
  ...
  <header>
    ...
    <h1 class="text-success p-4 m-5"> Contact From users </h1>
    <p>
    <a href="admin_desk.php" class="btn btn-success">Home</a>
</p>
