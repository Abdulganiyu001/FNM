...
if(!$id) {
    header('location: edit_book_action.php');
    exit;
}

$statement = $pdo->prepare('SELECT * FROM fem_proj3 WHERE id = :id');
...
$uploads_dir = './files';
move_uploaded_file($tname, $uploads_dir.'/'.$rname);

    }

                      //  $pname = $_FILES['file'] ? $rname : $book['file'];          
                        
                           // $rname = rand(1000,10000)."-".$_FILES["file"]["name"];
                            //     $tname = $_FILES["file"]["tmp_name"];
                            //     $uploads_dir = './files';
                            //     move_uploaded_file($tname, $uploads_dir.'/'.$rname);
                            ...
                            // if(!isset($_FILES['file'])){
                            //  // $pname = $_FILES['file'] ?? null;
                            //   $pname = $book['file'];
                            
                            // }
                          
                    $statement = $pdo-> prepare("UPDATE fem_proj3 SET title = :title, description = :description, file = :pname, pages = :pages WHERE id = :id");
...
<html lang="en">
  ...
  <header>
    ...
    <div class="container">  
          ...
          <h1 class="p-4 m-4 text-success"> Edit Book <?php echo $book['title'] ?> </h1>
          ...
          <?php if ($book['file']): ?>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" name="description" required> <?php echo $description //to return whats there when other fields are not filled ?></textarea>
            </div>
            ...
            <button type="submit" class="btn btn-success" name="submit">Edit Book</button>
    </form>
    </div>
